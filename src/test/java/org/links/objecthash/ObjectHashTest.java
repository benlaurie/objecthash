...
public class ObjectHashTest {
    ...
    public void testHashRedaction() throws JSONException, NoSuchAlgorithmException {
        ...
        String jsonFullWithRedacted = MessageFormat.format("'{'\"field3\": \"value3\", \"part\": {0}'}'", Redacted.pythonJsonHash(jsonPart));
    ...
    @Test
    public void testRedactedHashWorksSimilarToObjectHash() throws JSONException, NoSuchAlgorithmException {
        String jsonPart = "{\"field1\": \"value\", \"field2\": \"value2\"}";

        ObjectHash objectHash = ObjectHash.pythonJsonHash(jsonPart);
        ObjectHash redactedHash = Redacted.pythonJsonHash(jsonPart);

        assertNotEquals(objectHash.toString(), redactedHash.toString());
        assertEquals(objectHash.toString(), redactedHash.toString().replace(Redacted.PREFIX, ""));
    }
